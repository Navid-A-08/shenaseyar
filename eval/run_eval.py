...
def evaluate(golden_rows, retriever, catalog_ids, k=20):
    if k < 5:
        raise ValueError("k must be at least 5 to report Recall@5")
    invalid, missing, evaluated, ranks = [], [], [], []
    ...
    return {
        ...
        "rows": {"missing_from_catalog": missing, "invalid": invalid},
        "k": k,
        "metrics": {
...
def format_report(result, retriever_name):
    ...
    lines = [
        ...
        f"missing_from_catalog: {c['missing_from_catalog']}   invalid: {c['invalid']}",
    ]
